...
function InnerMessageBubble({ message, streaming }: MessageBubbleProps) {
  ...
  return (
    <div className={`flex items-start gap-2 md:gap-3 ${isUser ? 'flex-row-reverse' : ''}`}>
      ...
      `}>
        {isUser ? (
          <User className="h-3 w-3 md:h-4 md:w-4 text-slate-300" />
        ) : (
          <img 
      ...
      <div className={`flex-1 group ${isUser ? 'flex justify-end' : ''}`}>
        <div className={`
          max-w-[90%] md:max-w-[85%] rounded-xl md:rounded-2xl p-2 md:p-3 border break-words overflow-hidden text-sm md:text-base
          ${isUser 
        ...
        `}>
          {message.parts.map((part, index) => {
            ...
            if (isUser) {
              return <div key={index} className="whitespace-pre-wrap leading-relaxed">{text}</div>;
            }
        ...
        {!isUser && !streaming && (
          <div className="flex items-center gap-1 mt-1 md:mt-2 opacity-0 group-hover:opacity-100 transition-opacity">
            <Button
              variant="ghost"
              size="sm"
              className="h-6 w-6 md:h-7 md:w-7 p-0 text-slate-500 hover:text-slate-300"
              onClick={handleCopy}
              title="Copy message"
            >
